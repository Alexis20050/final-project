...
class MaintenanceRequestController extends Controller
{
    ...
    public function updateStatus(Request $request, MaintenanceRequest $maintenanceRequest)
    {
        ...
        if ($newStatus === 'resolved' && $oldStatus !== 'resolved') {
            $activeAllocation = Allocation::where('room_id', $room->id)->where('status', 'active')->exists();
    ...
    {
        // Only the resident who filed it can cancel, and only while pending
        if (auth()->id() !== $maintenanceRequest->user_id || $maintenanceRequest->status !== 'pending') {
            abort(403);
        }

        // Restore room status before deleting
        $room = $maintenanceRequest->room;
        ...
        if ($room) {
            ...
            $newRoomStatus = $activeAllocation ? 'occupied' : 'available';
            $room->update(['status' => $newRoomStatus]);
            Log::info("Room {$room->id} status restored to {$newRoomStatus} after cancelling request #{$maintenanceRequest->id}");
        }

        return redirect()->route('maintenance-requests.index')->with('success', 'Request cancelled.');
    }
}